<x-admin-layout>
    <x-slot name="header">
        <x-admin.page-header
            icon="user-plus"
            title="{{ __('admin.teams.add_member') }}"
            description="{{ __('admin.teams.create_team_member_info') }}"
            :breadcrumbs="[
                ['label' => __('admin.teams.title'), 'url' => route('admin.teams.index')],
                ['label' => __('admin.teams.add_member')]
            ]"
        >
            <x-slot:actions>
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <select name="role" 
                                    ...
                                    <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>{{ __('admin.teams.admin') }}</option>
                                </select>
                                @error('role')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror

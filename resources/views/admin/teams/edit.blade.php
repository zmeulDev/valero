    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-surface shadow-sm rounded-lg overflow-hidden">
                ...
                <div class="p-6 border-b border-border bg-background">
                    <div class="flex items-center space-x-6">
                        <div class="flex-shrink-0">
                            <img class="h-24 w-24 rounded-full object-cover border-4 border-surface shadow-md" 
                                 src="{{ $user->profile_photo_url }}" 
                                 alt="{{ $user->name }}">
                        </div>
                        <div>
                            <h3 class="text-2xl font-bold text-text">{{ $user->name }}</h3>
                            <div class="mt-1 text-sm text-muted space-y-1">
                ...
                <form action="{{ route('admin.teams.update', $user) }}" method="POST" class="divide-y divide-border">
                    @csrf
                    @method('PUT')
...
<div class="p-6 space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-surface rounded-lg border border-border overflow-hidden">
            <div class="p-4 border-b border-border">
                <div class="flex items-center justify-between">
                    <h4 class="text-sm font-medium text-text">{{ __('admin.teams.security') }}</h4>
                    <x-lucide-lock class="w-4 h-4 text-muted" />
                </div>
            </div>
            <div class="p-4">
                <div class="space-y-4">
                    <div>
                        <label for="password" class="block text-sm font-medium text-text">
                            ...
                            @if(isset($user))
                                <span class="text-muted text-xs ml-1">({{ __('admin.teams.leave_blank_to_keep_current') }})</span>
                            @endif
                        </label>
                        <div class="mt-1 relative rounded-lg shadow-sm">
                            ...
                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
                                <x-lucide-key class="h-5 w-5 text-muted" />
                            </div>
                        </div>
                        @error('password')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
        ...
        <div class="bg-surface rounded-lg border border-border overflow-hidden">
            <div class="p-4 border-b border-border">
                <div class="flex items-center justify-between">
                    <h4 class="text-sm font-medium text-text">{{ __('admin.teams.role_access') }}</h4>
                    <x-lucide-shield class="w-4 h-4 text-muted" />
                </div>
            </div>
            <div class="p-4">
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between p-4 bg-background rounded-lg">
                        <div>
                            <label for="is_active" class="text-sm font-medium text-text">{{ __('admin.teams.account_status') }}</label>

<x-admin-layout>
    <div x-data="{
        ...
        <x-slot name="header">
            <x-admin.page-header icon="bookmark" title="{{ __('admin.bookmarks.title') }}"
                description="{{ __('admin.bookmarks.description') }}" :breadcrumbs="[['label' => __('admin.bookmarks.breadcrumbs')]]">
                <x-slot:actions>
                ...
                <x-slot:stats>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <x-admin.stats-card icon="bookmark" label="{{ __('admin.bookmarks.total_bookmarks') }}"
                            :value="$bookmarks->total()" />
                        <x-admin.stats-card icon="tag" iconColor="blue" label="{{ __('admin.bookmarks.categories') }}"
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            ...
                            <form method="GET" action="{{ route('admin.bookmarks.index') }}" class="flex-1">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="search" name="search"
                                        ...
                                        class="block w-full pl-10 pr-3 py-2.5 border border-border rounded-lg leading-5 bg-background text-text placeholder-muted focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors duration-200">
                                </div>
                            </form>
                            ...
                            <div class="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0">
                                <a href="{{ route('admin.bookmarks.index', ['search' => request('search')]) }}"
                                    class="px-3 py-2 text-xs font-medium rounded-full whitespace-nowrap transition-colors duration-200 {{ !request('category') ? 'bg-indigo-600 text-white' : 'bg-background text-text hover:bg-surface border border-border' }}">
                                    {{ __('admin.bookmarks.all') }} ({{ $bookmarks->total() }})
                                </a>
                                @foreach($categories as $category)
                                    <a href="{{ route('admin.bookmarks.index', ['category' => $category, 'search' => request('search')]) }}"
                                        class="px-3 py-2 text-xs font-medium rounded-full whitespace-nowrap transition-colors duration-200 {{ request('category') === $category ? 'bg-indigo-600 text-white' : 'bg-background text-text hover:bg-surface border border-border' }}">
                                        {{ $category }}
                                    </a>
                                @endforeach
                            </div>
                ...
                @if($bookmarks->count() > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        @foreach($bookmarks as $bookmark)
                            <div
                                class="group bg-surface rounded-lg border border-border hover:border-indigo-300 dark:hover:border-indigo-600 hover:shadow-lg transition-all duration-200">
                                <div class="p-5">
                                    <!-- Header -->
                                    <div class="flex items-start justify-between mb-3">
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-2 mb-2">
                                                ...
                                                <h3 class="text-base font-semibold text-text truncate">
                                                    {{ $bookmark->title }}
                                                </h3>
                                            </div>
                                            @if($bookmark->category)
                                                <span
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300">
                                    ...
                                    @if($bookmark->link)
                                        <div class="mb-3">
                                            <div class="flex items-center gap-2 bg-background rounded-lg p-2.5 group/link">
                                                <x-lucide-link class="w-4 h-4 text-muted flex-shrink-0" />
                                                <div class="flex-1 min-w-0">
                                                ...
                                                <button @click="copyToClipboard('{{ $bookmark->link }}', {{ $bookmark->id }})"
                                                    type="button"
                                                    class="flex-shrink-0 p-1.5 rounded-md text-muted hover:text-indigo-600 hover:bg-indigo-50 dark:hover:text-indigo-400 dark:hover:bg-indigo-900/30 transition-colors duration-150">
                                                    <x-lucide-copy class="w-4 h-4" x-show="copiedId !== {{ $bookmark->id }}" />
                                                    <x-lucide-check class="w-4 h-4 text-green-600 dark:text-green-400"
                                                        x-show="copiedId === {{ $bookmark->id }}" />
                                                </button>
                                            </div>
                                            <div x-show="copiedId === {{ $bookmark->id }}" x-transition
                                    ...
                                    @if($bookmark->notes)
                                        <div class="mb-3">
                                            <div
                                                class="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800/50 rounded-lg p-2.5">
                                                <div class="flex items-start gap-2">
                                                    <x-lucide-sticky-note
                                                        class="w-4 h-4 text-yellow-600 dark:text-yellow-500 flex-shrink-0 mt-0.5" />
                                                    <p class="text-xs text-text leading-relaxed line-clamp-3">
                                                        {{ $bookmark->notes }}
                                                    </p>
                                                </div>
                                            </div>
                                        ...
                                        <div class="flex items-center gap-2">
                                            @if($bookmark->link)
                                                <button @click="copyToClipboard('{{ $bookmark->link }}', 'btn-{{ $bookmark->id }}')"
                                                    ...
                                                    <x-lucide-clipboard class="w-3.5 h-3.5 mr-1.5"
                                                        x-show="copiedId !== 'btn-{{ $bookmark->id }}'" />
                                                    <x-lucide-check class="w-3.5 h-3.5 mr-1.5 text-green-600"
                                                        x-show="copiedId === 'btn-{{ $bookmark->id }}'" />
                                                    <span
                                                        x-show="copiedId !== 'btn-{{ $bookmark->id }}'">{{ __('admin.bookmarks.copy_link') }}</span>
                                                    <span
                                                        x-show="copiedId === 'btn-{{ $bookmark->id }}'">{{ __('admin.bookmarks.copied') }}!</span>
                                                </button>
                                            @endif
                                            <a href="{{ route('admin.bookmarks.edit', $bookmark) }}"
                                                class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-text bg-background border border-border hover:bg-surface rounded-lg transition-colors duration-150">
                    ...
                    <!-- Pagination -->
                    <div class="bg-surface rounded-lg shadow-sm border border-border px-6 py-4">
                    ...
                    <div class="bg-surface shadow-sm rounded-lg border border-border">
                        <div class="text-center py-12">
                            <div class="inline-flex items-center justify-center w-16 h-16 bg-background rounded-full mb-4">
                                <x-lucide-bookmark class="w-8 h-8 text-muted" />
                            </div>
                            ...
                            @else
                                <a href="{{ route('admin.bookmarks.index') }}"
                                    class="inline-flex items-center px-4 py-2 border border-border shadow-sm text-sm font-medium rounded-lg text-text bg-surface hover:bg-background transition-colors duration-200">
        ...
        <div x-show="showDeleteModal" x-cloak class="fixed z-50 inset-0 overflow-y-auto" aria-labelledby="modal-title"
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300"
                    ...
                    aria-hidden="true" @click="closeDeleteModal()"></div>
                ...
                <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300"
                    ...
                    <div class="bg-background px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <form :action="`{{ route('admin.bookmarks.index') }}/${bookmarkToDelete?.id}`" method="POST"
                            class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                        ...
                        <button type="button" @click="closeDeleteModal()"
                            class="mt-3 w-full inline-flex justify-center rounded-md border border-border shadow-sm px-4 py-2 bg-surface text-base font-medium text-text hover:bg-background focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm transition-colors duration-200">

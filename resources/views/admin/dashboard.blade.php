    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    <!-- Quick Stats -->
                    <div class="bg-surface rounded-lg shadow-sm overflow-hidden border border-border transition-all duration-200 hover:shadow-md">
                        <div class="p-6">
                            <h2 class="text-xl font-semibold text-text flex items-center mb-6">
                            ...
                            <dl class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                                <div class="px-4 py-5 bg-background shadow-sm rounded-lg overflow-hidden sm:p-6 border border-border">
                                    <dt class="text-sm font-medium text-muted truncate">
                                        {{ __('admin.dashboard.avg_views_per_article') }}
                                    </dt>
                                    <dd class="mt-1 text-3xl font-semibold text-text">
                                        {{ $avgViewsPerArticle }}
                                    </dd>
                                </div>
                                <div class="px-4 py-5 bg-background shadow-sm rounded-lg overflow-hidden sm:p-6 border border-border">
                                    ...
                                    @foreach($topCategories as $category)
                                        <div class="group">
                                            <div class="flex items-center justify-between mb-1">
                                                <a href="{{ route('admin.categories.edit', $category) }}" class="text-sm font-medium text-text group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                                    {{ $category->name }}
                                                </a>
                                                <span class="text-xs font-semibold text-muted bg-background px-2 py-0.5 rounded-full border border-border">
                                        ...
                                        <li>
                                            <div class="relative pb-8">
                                                ...
                                                <div class="relative flex space-x-3">
                                                    <div>
                                                        <span class="h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-background {{ $activity['type'] === 'article_created' ? 'bg-indigo-500' : 'bg-green-500' }}">
                                                    ...
                                                    <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                                        <div>
                                                            <p class="text-sm text-muted">
                                                                @if($activity['type'] === 'article_created')
                                                                    ...
                                                                    <a href="{{ $activity['url'] }}" class="font-medium text-text hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                                                                        "{{ $activity['title'] }}"
                                                                    </a> 
                                                                    by <span class="text-text font-medium">{{ $activity['user'] }}</span>
                                                                @else
                                                                    <a href="{{ $activity['url'] }}" class="font-medium text-text hover:text-green-600 dark:hover:text-green-400 transition-colors">
                                                                        {{ $activity['user'] }}
                                                                    </a> 
                                                        ...
                                                        <div class="text-right text-sm whitespace-nowrap text-muted">
            ...
            <div class="mb-6 bg-surface rounded-lg shadow-sm overflow-hidden border border-border transition-all duration-200 hover:shadow-md"
                 x-data="{
                    ...
                    <div class="p-6">
                        <div class="flex justify-between items-center mb-6">
                            <h2 class="text-xl font-semibold text-text flex items-center">
                                ...
                                <span x-show="isLoading" x-transition class="ml-2 text-sm text-muted">
                                    <x-lucide-loader-2 class="w-4 h-4 animate-spin" />
                                </span>
                            </h2>
                            <div class="flex items-center space-x-4">
                                <div class="flex items-center bg-background rounded-lg p-1 border border-border">
                            ...
                            <table class="min-w-full divide-y divide-border">
                                <thead class="bg-background">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-muted uppercase tracking-wider">
                                ...
                                <tbody class="bg-surface divide-y divide-border">
                                    @foreach($topArticles as $index => $article)
                                        <tr class="hover:bg-background transition-colors duration-200 group">
                                            <td class="px-6 py-4">
                                                <div class="flex items-center">
                                                    <span class="text-xs font-medium text-muted mr-4 w-4 text-center">{{ $index + 1 }}</span>
                                                    ...
                                                    <div class="ml-4 min-w-0">
                                                        <a href="{{ route('admin.articles.edit', $article) }}" class="text-sm font-medium text-text hover:text-indigo-600 dark:hover:text-indigo-400 line-clamp-1 group-hover:underline decoration-2 underline-offset-2 decoration-indigo-200 dark:decoration-indigo-800">
                                                            {{ $article->title }}
                                                        </a>
                                                        <div class="text-xs text-muted mt-0.5">
                                                            by <span class="text-text">{{ $article->user->name }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>

<x-admin-layout>
    <x-slot name="header">
        <x-admin.page-header icon="pencil" title="{{ __('admin.playlists.edit_playlist') }}"
                ...
                <form action="{{ route('admin.playlists.update', $playlist) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="form-textarea block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50">{{ old('description', $playlist->description) }}</textarea>
                    </div>

                    <!-- Article Picker -->
                    <div class="mb-6"
                        x-data="articlePicker({{ $allArticles->map(fn($a) => ['id' => $a->id, 'title' => $a->title]) }}, {{ $playlist->articles->map(fn($a) => ['id' => $a->id, 'title' => $a->title]) }})">
                        <label
                    ...
                    <div class="flex justify-end">
                        <button type="submit"
                            ...
                            {{ __('admin.playlists.update_playlist') }}
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('articlePicker', (allArticles, initialSelected = []) => ({
                all: allArticles,

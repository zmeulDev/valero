<div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700" x-data="{
         bookmarks: [],
         allCategories: [],
         selectedCategory: '',
         searchQuery: '',
     ...
     }" x-init="init()" @category-changed.window="applyFilters()" @search-changed.window="applyFilters()">

    <!-- Header -->
    <div
        class="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-lg p-4 mb-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.bookmarks.create') }}" target="_blank"
    ...
    <div x-show="!loading && bookmarks.length === 0" x-cloak
        ...
        <h3 class="text-base font-medium text-gray-900 dark:text-white mb-2">{{ __('admin.bookmarks.no_bookmarks') }}
        </h3>
    ...
    <div x-show="!loading && bookmarks.length > 0" x-cloak>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 mb-4">
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <div class="flex-1">
                    <div class="relative">
                        ...
                        <input type="text" x-model.debounce.500ms="searchQuery" @input="applyFilters()"
                        ...
                        <button x-show="searchQuery" @click="searchQuery = ''; applyFilters()" type="button"
                ...
                <div class="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0">
                    <button @click="selectedCategory = ''; applyFilters()" type="button"
                    ...
                    <template x-for="category in allCategories" :key="category">
                        <button @click="selectedCategory = category; applyFilters()" type="button"
            ...
            <div x-show="total > 0" class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                <p class="text-xs text-gray-500 dark:text-gray-400">
                    {{ __('admin.bookmarks.showing') }} <span class="font-semibold text-gray-900 dark:text-white"
                        x-text="from"></span>
                    {{ __('admin.bookmarks.to') }} <span class="font-semibold text-gray-900 dark:text-white"
                        x-text="to"></span>
                    {{ __('admin.bookmarks.of') }} <span class="font-semibold text-gray-900 dark:text-white"
                        x-text="total"></span>
        ...
        <!-- No Results -->
        <div x-show="filteredBookmarks.length === 0" x-cloak
        ...
        <div x-show="filteredBookmarks.length > 0" class="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <template x-for="bookmark in filteredBookmarks" :key="bookmark.id">
                <div
                    class="group bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-600 hover:shadow-lg transition-all duration-200">
                    <div class="p-4">
                        <!-- Header -->
                        ...
                        <div x-show="bookmark.link" class="mb-3">
                            <div
                                class="flex items-center gap-2 bg-gray-50 dark:bg-gray-900/50 rounded-lg p-2.5 group/link">
                                <x-lucide-link class="w-4 h-4 text-gray-400 flex-shrink-0" />
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs text-gray-600 dark:text-gray-400 truncate" x-text="bookmark.link">
                                    </p>
                                </div>
                                <button @click="copyToClipboard(bookmark.link, 'link-' + bookmark.id)" type="button"
                                    ...
                                    <x-lucide-check class="w-4 h-4 text-green-600 dark:text-green-400"
                                        x-show="copiedId === 'link-' + bookmark.id" />
                                </button>
                            </div>
                            <div x-show="copiedId === 'link-' + bookmark.id" x-transition
                                ...
                                <div class="flex items-start gap-2">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <button x-show="bookmark.notes && bookmark.notes.length > 100"
                                            @click="toggleNotes(bookmark.id)" type="button"
                                            class="mt-1 text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-medium">
                                            <span
                                                x-show="!isNoteExpanded(bookmark.id)">{{ __('admin.bookmarks.show_more') }}</span>
                                            <span
                                                x-show="isNoteExpanded(bookmark.id)">{{ __('admin.bookmarks.show_less') }}</span>
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex items-center gap-2 pt-3 border-t border-gray-100 dark:border-gray-700">
                            <button x-show="bookmark.link" @click="copyToClipboard(bookmark.link, bookmark.id)"
                                type="button"
                            ...
                            <a :href="`{{ route('admin.bookmarks.index') }}/${bookmark.id}/edit`" target="_blank"
        ...
        <div x-show="lastPage > 1" class="mt-6 flex items-center justify-between">
            <div class="flex items-center gap-2">
                <button @click="changePage(1)" type="button" :disabled="currentPage === 1"
                ...
                <button @click="changePage(currentPage - 1)" type="button" :disabled="currentPage === 1"
                    :class="currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100 dark:hover:bg-gray-700'"
                    class="p-2 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 transition-colors duration-150">
            ...
            <div class="flex items-center gap-1">
                <template x-for="page in Array.from({length: lastPage}, (_, i) => i + 1).filter(p => {
                    ...
                    <div>
                        ...
                        <button @click="changePage(page)" type="button"
            ...
            <div class="flex items-center gap-2">
                <button @click="changePage(currentPage + 1)" type="button" :disabled="currentPage === lastPage"
                    :class="currentPage === lastPage ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100 dark:hover:bg-gray-700'"
                    class="p-2 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 transition-colors duration-150">
                    <x-lucide-chevron-right class="w-4 h-4" />
                </button>
                <button @click="changePage(lastPage)" type="button" :disabled="currentPage === lastPage"
                    :class="currentPage === lastPage ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100 dark:hover:bg-gray-700'"
                    class="p-2 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 transition-colors duration-150">
        ...
        <div class="mt-4 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
            ...
            <a href="{{ route('admin.bookmarks.index') }}" target="_blank"

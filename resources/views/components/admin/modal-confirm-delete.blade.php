...
<div x-data="{ 
    ...
    itemToDelete: null
     ...
     " x-show="showDeleteModal" class="fixed inset-0 overflow-y-auto z-50" x-cloak>
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="bg-background px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-border">
                <form method="POST"
                    :action="`{{ url('/admin/' . ($type === 'team member' ? 'teams' : ($type === 'category' ? 'categories' : $type . 's'))) }}/${itemToDelete}`">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                ...
                <button type="button" @click="showDeleteModal = false"
                    class="mt-3 w-full inline-flex justify-center rounded-md border border-border shadow-sm px-4 py-2 bg-surface text-base font-medium text-text hover:bg-background focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">

    ...
    <div class="bg-surface rounded-lg shadow-sm border border-border">
        <div class="p-4">
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <x-lucide-share-2 class="w-4 h-4 text-muted" />
            </div>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode($shareUrl) }}" target="_blank"
                    ...
                    class="flex items-center justify-center p-2 rounded-md bg-surface border border-border hover:bg-background group transition-colors">
                    <x-lucide-facebook class="w-5 h-5 text-muted group-hover:text-blue-500" />
                ...
                <a href="https://twitter.com/intent/tweet?url={{ urlencode($shareUrl) }}&text={{ urlencode($shareTitle) }}"
                    ...
                    class="flex items-center justify-center p-2 rounded-md bg-surface border border-border hover:bg-background group transition-colors">
                    <x-lucide-twitter class="w-5 h-5 text-muted group-hover:text-sky-500" />
                ...
                <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ urlencode($shareUrl) }}&title={{ urlencode($shareTitle) }}"
                    target="_blank" rel="noopener noreferrer"
                    class="flex items-center justify-center p-2 rounded-md bg-surface border border-border hover:bg-background group transition-colors">
                    <x-lucide-linkedin class="w-5 h-5 text-muted group-hover:text-blue-500" />
                ...
                <a href="https://api.whatsapp.com/send?text={{ urlencode($shareTitle) }}%20{{ urlencode($shareUrl) }}"
                    target="_blank" rel="noopener noreferrer"
                    class="flex items-center justify-center p-2 rounded-md bg-surface border border-border hover:bg-background group transition-colors">
                    <x-lucide-message-circle class="w-5 h-5 text-muted group-hover:text-green-500" />
                ...
                <button type="button" x-data="{ 
                    ...
                    <template x-if="!copied">
                        <x-lucide-link class="w-5 h-5 text-muted group-hover:text-indigo-500" />
                    </template>
                    <template x-if="copied">
                        <x-lucide-check class="w-5 h-5 text-green-500" />
                    </template>
                    <span class="sr-only"
                        x-text="copied ? '{{ __('frontend.common.link_copied') }}' : '{{ __('frontend.common.copy_link') }}'"></span>
                </button>
            </div>
        </div>
    </div>

@if($categories->count() > 0)
  ...
  <div x-data="sidebarSearch()" x-init="init()" class="bg-surface rounded-xl border border-border overflow-hidden mb-8">
    <!-- Search Header -->
    ...
    <div class="p-6">
      <form @submit.prevent="search" class="max-w-md mx-auto">
        <label for="sidebar-search-input"
          class="mb-2 text-sm font-medium text-text sr-only">{{ __('frontend.sidebar.search') }}</label>
        <div class="relative">
          <x-input type="search" id="sidebar-search-input" x-model="query"
            placeholder="{{ __('frontend.sidebar.search_placeholder') }}"
            class="block w-full p-4 ps-2 text-sm text-text border border-border rounded-lg bg-background focus:ring-blue-500 focus:border-blue-500 placeholder-muted" />
          ...
          <template x-for="article in articles.slice(0, 5)" :key="article.id">
            <li>
              <a :href="'/articles/' + article.slug"
                ...
                <!-- Accent Line -->
                <div
                  class="absolute left-0 top-0 bottom-0 w-0.5 bg-border group-hover:bg-indigo-500 dark:group-hover:bg-indigo-400 transition-colors duration-300">
                </div>

                <!-- Content -->
                <h5
                  class="text-sm font-medium text-text group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors duration-200 line-clamp-1"
      ...
      <div x-show="articles.length === 0 && hasSearched" class="mt-6 text-center py-8 px-4">

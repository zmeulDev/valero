...
<div class="space-y-8 p-4">
  @foreach ($articles as $article)
    <article
      class="group relative bg-surface rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border border-border">
      <div class="flex flex-col sm:flex-row p-5">
        <!-- Image Container -->
        <div class="sm:w-[280px] mb-4 sm:mb-0 sm:mr-6 flex-shrink-0">
          <div class="relative aspect-[4/3] overflow-hidden rounded-xl border border-border">
        ...
        <div class="flex-grow flex flex-col">
          <!-- Meta Information -->
          <div class="flex items-center gap-4 text-sm text-muted mb-3 pb-3 border-b border-border">
          ...
          <div class="mb-3">
            <span
              class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 rounded-full">
              {{ $article->category->name }}
            </span>
          </div>

          <h3
            class="text-xl font-bold text-text mb-3 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors duration-300">
            ...
            <div class="flex items-center gap-4">
              ...
              <div class="flex items-center gap-3">
                <img src="{{ $article->user->profile_photo_url }}" alt="{{ $article->user->name }}"
                  class="w-8 h-8 rounded-full object-cover ring-2 ring-surface border border-border">
              ...
              <a href="{{ route('category.index', $article->category->slug) }}"
                class="group/category flex items-center gap-1.5 text-sm font-medium text-muted hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors duration-200">
